...
const opcoesCores = [verdeCipreste, azulInverno, meiaNoite, estelar, rosaClaro]; //array sempre começa com zero
...
function atualizarCorSelecionada() {
  ...
  tituloProduto.classList.remove('verde-cipreste', 'azul-inverno', 'meia-noite', 'estelar', 'rosa-claro');
  tituloProduto.classList.add(opcoesCores[corSelecionada].nome.toLowerCase());
...
function atualizarTamanho() {
  ...
  //o charAt seleciona o caracter da posição que passamos, no caso o zero e exibe em tela exatamente o elemento que o usuário clicou (checked)
    
  tamanhoSelecionado = opcaoTamanhoSelecionado;
  if (opcoesTamanho[tamanhoSelecionado] === '41 mm') {
    imagemVisualizacao.classList.add('caixa-pequena');
    tituloProduto.innerText = `Pulseira loop esportiva ${opcoesCores[corSelecionada].nome.toLowerCase()}
     para caixa de ${opcoesTamanho[tamanhoSelecionado]}`;
    return; //o retorn retorna oque adicionamos e quando removemos, não precisa dele
  }
